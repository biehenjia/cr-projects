  ...
  switch ((*Subj).Type)
   {
    ...
    /*case TRIG_TANG   :
      case TRIG_COTANG :
      Тут надо бы предусмотреть следующее : если 1 - й и 3 - й аргументы -
      нули, то и все выражение - простой нуль.
		 break;*/
    default          :
		 Result=Subj;
		 break;
...
int GetToken(char* Arg)
 {
  ...
  int Result;
  char *SINUS_STR="sin";
  char *COSINUS_STR="cos";
  char *TANG_STR="tg";
  char *COTANG_STR="ctg";
  char *ARCSIN_STR="arcsin";
  char *ARCCOS_STR="arccos";
  char *ARCTANG_STR="arctg";
  char *ARCCOTANG_STR="arcctg";
  char *EXPONENT_STR="exp";
  char *LN_STR="ln";
  char *EMPTY_STR="          ";

  strcpy(Token,EMPTY_STR);
  Index=0;
  while ((c>='a')&&(c<='z'))
   {
    Token[Index]=c;
  ...
  Token[Index]=0;
  if (!strcmp(Token,SINUS_STR)) Result=SINUS; else
  if (!strcmp(Token,COSINUS_STR)) Result=COSINUS; else
  if (!strcmp(Token,TANG_STR)) Result=TANG; else
  if (!strcmp(Token,COTANG_STR)) Result=COTANG; else
  if (!strcmp(Token,ARCSIN_STR)) Result=ARCSIN; else
  if (!strcmp(Token,ARCCOS_STR)) Result=ARCCOS; else
  if (!strcmp(Token,ARCTANG_STR)) Result=ARCTANG; else
  if (!strcmp(Token,ARCCOTANG_STR)) Result=ARCCOTANG; else
  if (!strcmp(Token,EXPONENT_STR)) Result=EXPONENT; else
  if (!strcmp(Token,LN_STR)) Result=LN; else
   if (Index==1)
    {
     Result=VARIABLE;
     NewVariable=Token[0];
  ...
  while ((c!=0)&&(!Error))
   {
    switch (c)
     {
      ...
      case '-':
	       ...
	       if (LastLex==NUMBER||
		   LastLex==VARIABLE||
		   LastLex==RIGHT) LastLex=B_MINUS;
		else
		 if (LastLex==LEFT||
		     LastLex==0) LastLex=U_MINUS;
		  else Error=-4;
	       break;
      case '*':
	       c=Arg[Count];
      ...
      case '9':
	       i=GetDigit(Arg);
	       if (LastLex!=RIGHT) LastLex=NUMBER;
		else Error=-4;
	       break;
      default:
	       if (LastLex==PLUS||
		   ...
		   LastLex==POWER||
		   LastLex==LEFT||
		   LastLex==0) LastLex=GetToken(Arg);
		else Error=-4;
	       break;
     };
    ClearError();
    if (!Error)
     switch (LastLex)
      {
       case NUMBER:
		   TResult=FormDigitCR(i);
		   if (!Error) PushExpr(TResult);
		   break;
       case VARIABLE:
		   TResult=FormVarCR(NewVariable);
		   ClearError();
		   if (!Error) PushExpr(TResult);
		   break;
       case PLUS:
       ...
       case RIGHT:
       case U_MINUS:
		   while(Enable(FormOpExpr(LastLex))&&(!Error)&&(!EmptyAction())&&(!Interrupt))
		    {
		     switch ((*((*Action).Act)).Type)
		      {
		       case PLUS:
				 ...
				 if (LookExpr2())
				  {
				   ...
				   FOper=PopExpr();
				   if (!Error)
				   SOper=MulCR(FormDigitCR(-1),SOper);
				   if (!Error)
				   TResult=AddCR(FOper,SOper);
				   if (!Error)
				   PushExpr(TResult);
				 ...
				 if (LookExpr1())
				  {
				   ...
				   CurrAct=PopAct();
				   if (!Error)
				   TResult=ApplyCRRule((*CurrAct).Type,SOper);
				   if (!Error)
				   PushExpr(TResult);
				   if (!Error)
				    {
				     DestroyCR(SOper);
                                     Destroy(CurrAct,CurrAct);
				    };
				  } else Error=-4;
				 break;
		       case LEFT: PopAct();Interrupt=1;break;
		      };
		    };
		   if (!Error) CurrAct=FormOpExpr(LastLex);
		   if ((!Error)&&(!Interrupt)) PushAct(CurrAct);
		   break;
       default: Error=-4;
		break;
      };
		 ...
		 if (LookExpr1())
		  {
		   SOper=PopExpr();
		   if (!Error)
		   TResult=ApplyCRRule((*CurrOp).Type,SOper);
		   if (!Error)
		   PushExpr(TResult);
		   if (!Error)
		    {
    ...
    ClearError();
    if ((!LookExpr2())&&(LookExpr1())&&(!Error)&&(EmptyAction()))
     TResult=PopExpr();
     else
      {
       Count--;
       printf("Error occured in position %d\n",Count);
       Speller();
      };
    while (!EmptyAction())
     {
...
void ShowCR( CR* Subj,FILE* outp)
 {
  int Count;
  if (CRType(Subj)==NUMBER)
   fprintf(outp,"%f",(*Subj).Value);
  else
   {
    fprintf(outp,"CR[%d %c] %d (",CRType(Subj),(Variables[MVar(Subj)]).Name,(*Subj).Length);
    for (Count=0;Count<(*Subj).Length-1;Count++)
     {
      ShowCR((*((*Subj).Args))[Count],outp);
      fprintf(outp,",");
     };
    ShowCR((*((*Subj).Args))[(*Subj).Length-1],outp);
    fprintf(outp,")");
   }
 }
